...
public final class SparkDialect extends SqlDialect {
    private final SparkStorage storage;

    public SparkDialect(SparkStorage storage) {
    ...
    public Optional<String> timePeriod(Granularity granularity) {
        ...
        if (timeType == TimeType.TS_SECOND) {
            datetimeExpr = String.format("to_timestamp(%s)", storage.getEventTimeField());
        } else if (timeType == TimeType.TS_MILLI) {
            datetimeExpr = String.format("to_timestamp(%s /1000)", storage.getEventTimeField());
        } else if (timeType == TimeType.FORMAT) {
            datetimeExpr = String.format("to_timestamp(cast(%s as string),'%s')", storage.getEventTimeField(), timeFieldType);
        } else {
