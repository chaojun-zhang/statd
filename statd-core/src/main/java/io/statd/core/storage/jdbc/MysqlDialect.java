...
public class MysqlDialect extends SqlDialect {
    ...
    public Optional<String> timePeriod(Granularity granularity) {
        ...
        switch (granularity) {
            case GMin:
                return Optional.of(String.format("from_unixtime(unix_timestamp(%s) - unix_timestamp(%s) %s)", timestampField, timestampField, "% 60"));
            case G5min:
                return Optional.of(String.format("from_unixtime(unix_timestamp(%s) - unix_timestamp(%s) %s)", timestampField, timestampField, "% 300"));
            case GHour:
                return Optional.of(String.format("from_unixtime(unix_timestamp(%s) - unix_timestamp(%s) %s)", timestampField, timestampField, "% 3600"));
            case GDay:
