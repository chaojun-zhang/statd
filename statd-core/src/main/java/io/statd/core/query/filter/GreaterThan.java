...
public final class GreaterThan implements FieldFilter {
    ...
    public void validate(Query search) {
        if (field == null) {
            throw new StatdException(QueryRequestError.InvalidFilter, "field is required for GreaterThan filter");
        }
        if (value == null) {
            throw new StatdException(QueryRequestError.InvalidFilter, "value is required for GreaterThan filter, field:'" + field + "'");
        }
    }
