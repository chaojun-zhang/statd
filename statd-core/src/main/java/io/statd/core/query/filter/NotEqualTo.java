...
public final class NotEqualTo implements FieldFilter {
    ...
    public void validate(Query search) {
        if (field == null) {
            throw new StatdException(QueryRequestError.InvalidFilter, "field is required for NotEqualTo filter");
        }
        if (value == null) {
            throw new StatdException(QueryRequestError.InvalidFilter, "value is required for NotEqualTo filter, field:'" + field + "'");
        }
    }
