...
public final class LessThan implements FieldFilter {
    ...
    public void validate(Query search) {
        if (field == null) {
            throw new StatdException(QueryRequestError.InvalidFilter, "field is required for LessThan filter");
        }
        if (value == null) {
            throw new StatdException(QueryRequestError.InvalidFilter, "value is required for LessThan filter, field:'" + field + "'");
        }
    }
